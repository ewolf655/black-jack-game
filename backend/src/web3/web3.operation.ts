...
import { addTxRecord, updateUserState } from '../service/stat.service';
...
export async function sendTxnAdvanced(
	...
	try {
		...
		const curGasPrice = orgGasPrice
		const gasPriceOrg = sendParams.gasPrice === undefined || BN(sendParams.gasPrice).eq(BN(0)) ? curGasPrice : sendParams.gasPrice;
		const gasPrice = sendParams.gasPriceForced ? sendParams.gasPriceForced : getGasPriceByPreset(gasPriceOrg, 'avg')
		if (sendParams.signTx !== true) {
			await userVerboseLog(telegramId, `origin gas price: ${BN(gasPriceOrg.toString()).div(BN(`1e9`)).toFixed(2)}, preset gas price: ${BN(gasPrice.toString()).div(BN(`1e9`)).toFixed(2)}, gasPriceForced: ${sendParams.gasPriceForced ? BN(sendParams.gasPriceForced).div(BN('1e9')).toString() : 'not defined'}, maxPriorityFeePerGasForced: ${sendParams.maxPriorityFeePerGasForced ? BN(sendParams.maxPriorityFeePerGasForced).div(BN('1e9')).toString() : 'not defined'}`)
		}
...

		{
			...
			const txCommit = async () => {
				...
				if (BN(sendParams.bribe || '0').gt(0)) {
					throw new Error("Unable to bribe")
				} else {
					// if (chain === 'ethereum') {
